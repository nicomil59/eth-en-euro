...
const getCurrencies = async () => {
    ...
    try {
        ...
        const ethRateInEUR = marketData.current_price.eur;
        ...
        priceOneETH.innerText = `€ ${marketData.current_price.eur.toFixed(2)}`;

        // Variation du prix sur 24h

        const variationPercentOneDay = marketData.price_change_percentage_24h_in_currency.eur.toFixed(2);
        ...
        priceOneETH24hValue.innerText = `Variation / 24h - Valeur : € ${(marketData.current_price.eur - marketData.price_change_24h_in_currency.eur).toFixed(2)}`;

        // Variation du prix sur 7, 30 et 200 jours

        priceOneWeek.innerHTML = `€ ${((marketData.current_price.eur * 100) / (100 + marketData.price_change_percentage_7d_in_currency.eur)).toFixed(2)} <span>${marketData.price_change_percentage_7d_in_currency.eur > 0 ? "+": ""}${marketData.price_change_percentage_7d_in_currency.eur.toFixed(2)}%</span>`;
        
        priceOneWeek.lastChild.style.color = marketData.price_change_percentage_7d_in_currency.eur > 0 ? "green" : "red";

        priceOneMonth.innerHTML = `€ ${((marketData.current_price.eur * 100) / (100 + marketData.price_change_percentage_30d_in_currency.eur)).toFixed(2)} <span>${marketData.price_change_percentage_30d_in_currency.eur > 0 ? "+": ""}${marketData.price_change_percentage_30d_in_currency.eur.toFixed(2)}%</span>`;

        priceOneMonth.lastChild.style.color = marketData.price_change_percentage_30d_in_currency.eur > 0 ? "green" : "red";

        price200days.innerHTML = `€ ${((marketData.current_price.eur * 100) / (100 + marketData.price_change_percentage_200d_in_currency.eur)).toFixed(2)} <span>${marketData.price_change_percentage_200d_in_currency.eur > 0 ? "+": ""}${marketData.price_change_percentage_200d_in_currency.eur.toFixed(2)}%</span>`;

        price200days.lastChild.style.color = marketData.price_change_percentage_200d_in_currency.eur > 0 ? "green" : "red";
